...
public class MyComputeFactorialUseCase extends BaseObservable<MyComputeFactorialUseCase.Listener> {

    public interface Listener {
        void onComputeFactorialCompleted(String resultString);
    }
    ...
    public void computeFactorial(final int factorialArgument, final int timeout) {
        new Thread(() -> {
            initComputationParams(factorialArgument, getTimeout(timeout));
            startComputation();
    ...
    private void processComputationResults() {
        ...
        mUiHandler.post(() -> {
            for (MyComputeFactorialUseCase.Listener listener : getListeners()) {
                listener.onComputeFactorialCompleted(finalResultString);
            }
        });
    }
