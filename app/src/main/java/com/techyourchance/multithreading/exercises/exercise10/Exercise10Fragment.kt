    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        btnStartWork.setOnClickListener { _ ->
            ...
            job = CoroutineScope(Dispatchers.Main).launch {
                val uiState = computeFactorialUseCase.computeFactorialAndNotify(argument, getTimeout())
                when(uiState) {
                    is SUCCESS -> onFactorialComputed(uiState.result)
                    TIMEOUT -> onFactorialComputationTimedOut()
                }
            }
        }
